...
export default function ProductGallery({
    ...
    const [ syncedVariant, setSyncedVariant ] = useState(variantIndex);

    const mainRef = useRef<HTMLDivElement>(null);

    // Reset activeIndex when variant changes, but defer to next render
    if (syncedVariant !== variantIndex) {
        setSyncedVariant(variantIndex);
        setActiveIndex(0);
    }
    ...
    return (
        <section className={`flex w-full flex-col gap-4 lg:flex-row ${className}`}>
            <div className="order-2 flex gap-3 overflow-x-auto lg:order-1 lg:flex-col">
                {images.map((src, i) => (
                    <button className={`relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg ring-1 ring-light-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[--color-dark-500] ${i === activeIndex ? "ring-[--color-dark-500]" : ""}`}
                    key={`${src}-${i}`}
