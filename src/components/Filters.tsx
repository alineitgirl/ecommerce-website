...
return (
  ...
  <div className='mb-4 flex items-center justify-between md:hidden'>
    <button className='rounded-md border border-light-300 px-3 py-2 text-body-medium'
        ...
        <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true">
          ...
          <div className="absolute inset-y-0 left-0 w-80 max-w-[80%] overflow-auto bg-light-100 p-4 shadow-xl">
            ...
            <div className="md:hidden">
              <Group title="Gender" k="gender" expanded={expanded} setExpanded={setExpanded}>
                <ul className="space-y-2">
                  {GENDERS.map((g) => {
                    const checked = getArrayParam(search, "gender").includes(g);
                    const genderLabel = g === 'men' ? 'Men' : g === 'women' ? 'Women' : 'Unisex';
              ...
              <Group title="Size" k="size" expanded={expanded} setExpanded={setExpanded}>
              ...
              <Group title="Color" k="color" expanded={expanded} setExpanded={setExpanded}>
                <ul className="grid grid-cols-2 gap-2">
                  {COLORS.map((c) => {
                    const checked = getArrayParam(search, "color").includes(c);
                    const colorLabel = c === 'black' ? 'Black' : c === 'white' ? 'White' : c === 'red' ? 'Red' : c === 'green' ? 'Green' : c === 'blue' ? 'Blue' : 'Grey';
              ...
              <Group title="Price" k="price" expanded={expanded} setExpanded={setExpanded}>
                <ul className="space-y-2">
                  {PRICES.map((p) => {
                    const checked = getArrayParam(search, "price").includes(p.id);
                    return (
