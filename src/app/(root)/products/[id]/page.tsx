...
import { getProduct, getProductReviews, type Review, getRecommendedProducts, type RecommendedProduct} from "@/src/lib/actions/product";
        ...
        {reviews.length === 0 ? (
            ...
            <ul className="space-y-4">
                {reviews.slice(0, 10).map((r) => (
                    <li className="rounded-lg border border-light-300 p-4"
                    key={r.id}>
...
export async function AlsoLikeSection({ productId } : { productId: string }) {
...
export default async function ProductDetailPage({ params } : {params : Promise<{id: string}>}) {
    ...
    v.id === product.defaultVariantId || variants[0]);

    const basePrice = defaultVariant ? Number(defaultVariant.price) : null;
    const salePrice = defaultVariant ? Number(defaultVariant.salePrice) : null;
    ...
    return (
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <nav className="py-4 text-caption text-dark-700">
                <Link href="/" className="hover:underline">Home</Link>
                <Link href="/products" className="lg:sticky lg:top-6" />
            ...
            <section className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_480px]">
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-3">
                        <button className="flex items-center justify-center gap-2 rounded-fill bg-dark-900 px-6 py-4 text-body-medium text-light-100 transition hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[--color-dark-500]">
                            <ShoppingBag className="h-5 w-5" />
                            Add to Bag
                        </button>

                        <button className="flex items-center justify-center gap-2 rounded-full border border-light-300 px-6 py-4 text-body-medium text-dark-900 transition 
                        hover:border-dark-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-[--color-dark-500]">
                    ...
                    <CollapsibleSection title="Shipping & Returns">
                        <p>Free standard shipping and free 30-day retuns for Nike members.</p>
                    </CollapsibleSection>
