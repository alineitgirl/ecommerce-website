...
export function stringifyQuery(query:QueryObject) {
    return qs.stringify(query, { arrayFormat: "bracket", skipEmptyString: true, skipNull: true });
}

export function withUpdateParams(pathname: string, currentSearch: string, updates: QueryObject) : string {
...
export function toggleArrayParam(pathname: string, currentSearch: string, key : string, value : string) : string {
    ...
    const updates: QueryObject = {
        [key]: nextValues.length === 0 ? undefined : nextValues };

    return withUpdateParams(pathname, currentSearch, updates);
}
...
export function setParam(
    ...
    return withUpdateParams(pathname, currentSearch, { [key]: value  === null || value === undefined ? undefined : String(value) });
}
...
export function parseFilterParams(sp : Record<string, string | string[] | undefined>) : NormalizedProductFilters {
    const getArr = (k : string) => {
        ...
        const arr2 = Array.isArray(v2) ? v2.map(String) : v2 === undefined ? [] : [String(v2) as string];
        return [...arr1, ...arr2];
    };
    ...
    const genderSlugs = getArr("gender").map((g) => g.toLowerCase());
    const brandSlugs = getArr("brand").map((b) => b.toLowerCase());
    const categorySlugs = getArr("category").map((c) => c.toLowerCase());
    const sizeSlugs = getArr("size").map((s) => s.toLowerCase());
    const colorSlugs = getArr("color").map((c) => c.toLowerCase());

    const priceRangeStr = getArr("price");
    const priceRanges : Array<[number | undefined, number | undefined]> = priceRangeStr.map((pr) => {
        const [minStr, maxStr] = String(pr).split("-");
    ...
    sortParam === "price-asc" || sortParam === "price-desc" || sortParam === "featured" || sortParam === "newest"  
    ? sortParam
    : "newest";
